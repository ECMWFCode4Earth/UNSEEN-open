...
```{r}
EOBS_UK_weighted_upscaled_df <- read.csv("../../Data/EOBS_UK_weighted_upscaled_df.csv", row.names = 1)
SEAS5_UK_weighted_masked_df <- read.csv("../../Data/SEAS5_UK_weighted_masked_df.csv", row.names = 1)
``` 

```{r}
ensemble <- SEAS5_UK_weighted_masked_df
obs <- EOBS_UK_weighted_upscaled_df
obs_same_period <- EOBS_UK_weighted_upscaled_df[EOBS_UK_weighted_upscaled_df$Year > 1980 & EOBS_UK_weighted_upscaled_df$Year < 2016,]
ggplot() +
...
Boottest(ensemble = SEAS5_UK_weighted_masked_df$Precipitation, obs = EOBS_UK_weighted_upscaled_df$Precipitation, xlabel = "Precipitation")
```
...
Boottest_biascor(ensemble = SEAS5_UK_weighted_masked_df$Precipitation, obs = EOBS_UK_weighted_upscaled$Precipitation, fontsize = 11) # %>%
...
EVT_plot <- function(obs, ensemble, GEV_type, main, ylim = NA) {
  ...
  ggplot(data = datapoints_S5, aes(x = rp_S5)) +
    ...
    labs(title = main, y = "February average precipitation (mm/day)", x = "Return period (years)") +
    if (is.finite(ylim)) {
      coord_cartesian(ylim = c(0, ylim))
    }
}

GEV1 <- EVT_plot(ensemble = SEAS5_UK_weighted_masked_df$Precipitation, obs = EOBS_UK_weighted_upscaled_df$Precipitation, main = "GEV", GEV_type = "GEV") # %>%
# ggsave(filename = "graphs/ERA5Gev_SEAS5corrections_region1.png",width =180,height = 180, units='mm',dpi=300)
Gumbel1 <- EVT_plot(ensemble = SEAS5_UK_weighted_masked_df$Precipitation, obs = EOBS_UK_weighted_upscaled_df$Precipitation, main = "Gumbel", GEV_type = "Gumbel") # %>%
